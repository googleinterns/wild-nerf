...
def posenc(x, min_deg, max_deg, legacy_posenc_order=False, alpha=1.0):
  ...
  # Eq. 14 in BARF (https://arxiv.org/pdf/2104.06405.pdf)
  # w is the smoothing mask at different frequency bands.
  L = scales.shape[0]
  a = jnp.clip(alpha, 0, 1.0) * L
  k = jnp.arange(L)
  w = jnp.zeros_like(k, dtype=jnp.float32)
  mid = (0 <= (a - k)) & ((a - k) < 1)
  w = jnp.where(mid, ((1 - jnp.cos((a - k) * jnp.pi)) / 2), w)
  w = jnp.where(a - k >= 1, jnp.ones_like(w), w)

  if legacy_posenc_order:
    raise NotImplementedError(
        'BARF positional encoding not implemented for `legacy_posenc_order`.')
    ...
    w = jnp.repeat(w, 2*x.shape[-1], axis=-1)
    four_feat = four_feat * w
...
@jax.jit
def unravel_index_multi_2d(flat_inds, shapes):
  """Similar to NumPy's unravel_index, but accepts multiple arbitrarily sized
  2D array shapes.

  Args:
    flat_inds: An integer array of shape (B,) containing linear indices into
      contiguous arbitarily sized arrays. It does not check for out-of-bounds
      values.
    shapes: An integer array of shape (N, 2).

  Returns:
    x: An integer array of shape (B,).
    y: An integer array of shape (B,).
    array_inds: An integer array of shape (B,). Indicates which array the
      indices correspond to.

  """
  intervals = shapes.prod(axis=-1).cumsum()
  array_inds = intervals.searchsorted(flat_inds, side='right')
  local_inds = flat_inds - jnp.concatenate(
      [jnp.array([0]), intervals])[array_inds]
  widths = shapes[array_inds, -1]
  y = local_inds // widths
  x = local_inds % widths
  return array_inds, y, x


def generate_rays(flat_indices, camtoworlds, image_hw, cx, cy, fx, fy):
  """Generate rays on-the-fly (instead of selecting from a precomputed pool).

  The arguments can be learnable variables. This function returns the same ray
  values as the precomputed version given index i and also supports per-image
  parameters.

  Args:
    flat_indices: An integer array of shape (B,) with valid values
      0<=x<number_of_pixels. The pixel value of ray i is given by
      `jnp.concatenate([img.reshape(-1, 3) for img in images], axis=0)[i]`
    camtoworlds: An array of shape (N, 3, 4).
    image_hw: An array of shape (N, 2).
    cx: An array of shape (N,).
    cy: An array of shape (N,).
    fx: An array of shape (N,).
    fy: An array of shape (N,).

  Returns:
    rays: utils.Rays namedtuple containing (B, 3) 3D coordinates.
    cam_idx: which camera the ray came from. (B,)

  """
  num_cameras = camtoworlds.shape[0]
  assert num_cameras == cx.shape[0]
  assert num_cameras == cy.shape[0]
  assert num_cameras == fx.shape[0]
  assert num_cameras == fy.shape[0]
  assert num_cameras == image_hw.shape[0]

  # 0.5 is correct. Original NeRF paper used 0.0.
  pixel_center = 0.5
  # (B,), (B,), (B,)
  cam_idx, y, x = unravel_index_multi_2d(flat_indices, image_hw)
  # (B, 3)
  camera_dirs = jnp.stack([(x + pixel_center - cx[cam_idx]) / fx[cam_idx],
                           -(y + pixel_center - cy[cam_idx]) / fy[cam_idx],
                           -jnp.ones_like(x)], axis=-1)
  # (B, 3)
  world_dirs = (camera_dirs[..., None, :] *
                camtoworlds[cam_idx, :3, :3]).sum(axis=-1)

  # (B, 3)
  origins = camtoworlds[cam_idx, :3, 3]

  # (B, 3)
  viewdirs = world_dirs/jnp.linalg.norm(world_dirs, axis=-1, keepdims=True)

  rays = utils.Rays(origins=origins, directions=world_dirs, viewdirs=viewdirs)
  return rays, cam_idx
